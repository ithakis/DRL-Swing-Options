#!/bin/bash
# v22: soften PER, keep exploration alive longer with higher floor and slower decay to avoid boundary lock-in; keep 128 batch.
args=(
    ...
    -nstep=1
    --per_alpha=0.45           # Softer PER prioritization to reduce over-focus
    ...
    --noise_sigma=1.3          # Stronger initial noise scale for early exploration
    --noise_anneal_power=0.55  # Tie noise std to epsilon with gentler annealing
    --noise_plateau=2500       # Hold initial noise/epsilon longer to avoid early decay
    --min_action_noise=0.22    # Higher noise floor to prevent variance collapse
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99996     # Slower decay; retains exploration deeper into training
    ...
    --final_lr_fraction=0.85   # Keep more learning rate for late-stage adjustment
...
python run.py "${args[@]}" -name "SwingOption_20_v22_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v22_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v22_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v22_14" -seed 14
