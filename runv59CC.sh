...
#   - Fix 2: Pre-Activation Entropy Regularization. Penalizes |tanh(u)| -> 1 saturation directly in the actor loss.
#     Prevents "Gradient Death" by keeping pre-activation values in the linear region.
# - v58 Feature details: Rprop Calibration (robust init), Stratified Sampling (stable buffers).

args=(
    # Same scale as v57/v58: 32k training episodes
    -n_paths=32768
    ...
    --critic_warmup_episodes=1024 # Freeze actor updates for 1024 episodes (approx 22k steps)
    --entropy_reg=0.01            # Penalize saturation to keep gradients alive (pre-activation reg)
...
python run.py "${args[@]}" -name "SwingOption_20_v59_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v59_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v59_13" -seed 13

# Run Convex Cost variants (CC)
python run.py "${args[@]}" -name "SwingOption_20_v59CC_11" -seed 11 --c_cost 0.15 --gamma_cost 2.0 &
python run.py "${args[@]}" -name "SwingOption_20_v59CC_12" -seed 12 --c_cost 0.15 --gamma_cost 2.0 &
python run.py "${args[@]}" -name "SwingOption_20_v59CC_13" -seed 13 --c_cost 0.15 --gamma_cost 2.0
