#!/bin/bash
# v64 canonical convex-cost experiment: c_cost=0.15, gamma_cost=2
...
args=(
    ...
    --c_cost=0.15              # convex exercise-cost coefficient (this cell)
    --gamma_cost=2          # convex-cost exponent (this cell)
    --disable_csv_logging=0
    --limit_logging_frequency=1
)
pids=()
for seed in 11 12 13; do
    python run.py "${args[@]}" -name "SwingOption_20_c0.15_gamma2_${seed}" -seed "$seed" &
    pids+=($!)
done
exit_code=0
