#!/bin/bash
# v64 canonical convex-cost experiment: c_cost=0.01, gamma_cost=3
...
args=(
    ...
    --gamma_cost=3          # convex-cost exponent (this cell)
    --disable_csv_logging=0
    --limit_logging_frequency=1
)
pids=()
for seed in 11 12 13; do
    python run.py "${args[@]}" -name "SwingOption_20_c0.01_gamma3_${seed}" -seed "$seed" &
    pids+=($!)
done
exit_code=0
