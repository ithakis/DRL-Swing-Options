#!/bin/bash
# v64 canonical convex-cost experiment: c_cost=0.10, gamma_cost=1
...
args=(
    ...
    --c_cost=0.10              # convex exercise-cost coefficient (this cell)
    --gamma_cost=1          # convex-cost exponent (this cell)
    --disable_csv_logging=0
    --limit_logging_frequency=1
)
pids=()
for seed in 11 12 13; do
    python run.py "${args[@]}" -name "SwingOption_20_c0.10_gamma1_${seed}" -seed "$seed" &
    pids+=($!)
done
exit_code=0
