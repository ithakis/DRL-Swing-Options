#!/bin/bash
# v28: Sigmoid PER ramp, early behavior closer to NP1, revert to v26-like noise/decay, and moderate action reg to retain late performance.
args=(
    ...
    --per_alpha_final=0.35     # Slightly lower final alpha to avoid aggressive focus
    --per_alpha_ramp_start=3000  # Start PER alpha ramp after ~3k episodes
    --per_alpha_ramp_end=14000   # End PER alpha ramp by ~14k episodes
    --per_beta_final=0.85      # Keep beta higher for more IS correction late
    ...
    --final_lr_fraction=0.95   # Back to v26 LR decay
...
python run.py "${args[@]}" -name "SwingOption_20_v28_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v28_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v28_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v28_14" -seed 14
