"""
H7: TD3-style twin critics + H1 kernel-expected target.

Tests whether adding a second critic and taking the min over both kernel-
expected predictions reduces overestimation bias on top of H1's variance
reduction.

Configs (focal, 4096 ep, 3 seeds):
  CTRL_proper       no kernel, no twin critic
  H1_only           kernel only (Phase 1 winner)
  H7                H1 + twin critic

Reuses the base config from sweep_expected_target.

Usage:
    python tools/sweep_h7.py --max_workers 3 --seeds 11 12 13
"""
...
PHASE_CONFIGS = {
    "CTRL_proper":   {**KERNEL_OFF, "--use_twin_critic": "0"},
    "H1_only":       {**KERNEL_ON_NO_WARMUP, "--use_twin_critic": "0"},
    "H7":            {**KERNEL_ON_NO_WARMUP, "--use_twin_critic": "1"},
}
...
def main() -> None:
    ...
    print(f"H7 sweep: {len(runs)} runs at max_workers={args.max_workers}")

    csv_path = LOG_DIR / f"sweep_h7_n{args.n_paths}.csv"
    ...
    def execute(run: Dict) -> Dict:
        ...
        row = run_one(cfg, base, args.dry_run, tag_suffix="_h7")
        row.update({"tag": "_h7", "n_paths": str(run["n_paths"]), "contract": "focal"})
        return row
