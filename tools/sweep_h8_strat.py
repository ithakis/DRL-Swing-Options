"""H8 stratify-preserved sweep: 3 seeds at 4096 ep."""
...
PHASE_CONFIGS = {
    "H8_strat": {**KERNEL_ON, "--use_twin_critic": "0",
                 "--use_antithetic_target": "1", "--antithetic_preserve_stratify": "1",
                 "--use_jump_iw": "0"},
}
...
def main():
    ...
    print(f"H8-strat sweep: {len(runs)} runs at max_workers={args.max_workers}")
    csv_path = LOG_DIR / f"sweep_h8strat_n{args.n_paths}.csv"
    ...
    def execute(run):
        ...
        row = run_one(cfg, base, False, tag_suffix="_h8s")
        row.update({"tag": "_h8s", "n_paths": str(run["n_paths"]), "contract": "focal"})
        return row
