Greetings G
#!/bin/bash
# v31: Slightly stronger late PER and higher noise floor to restore v26-level pricing while keeping v30 stability; shorter action L2 window to keep action variance up.
args=(
    ...
    -nstep=1
    --per_alpha=0.05           # PER extremely soft to mimic uniform early
    --per_beta_start=1.0       # Full IS correction (uniform effect early)
    --per_beta_frames=150000   # Very slow anneal (keeps beta ~1 through mid-run)
    --per_priority_floor=5e-6  # Minimal floor
    --per_priority_clip_pct=0  # No clipping
    --per_alpha_final=0.34     # Slightly stronger late PER than v30 to lift TD signal
    --per_alpha_ramp_start=3200  # Start PER alpha ramp after ~3.2k episodes
    --per_alpha_ramp_end=15000   # End PER alpha ramp by ~15k episodes
    --per_beta_final=0.9       # Keep beta higher for stronger IS correction late
    --per_alpha_sigmoid=1      # Use sigmoid ramp for alpha/beta
    ...
    --min_action_noise=0.20    # Higher floor to preserve action variance and avoid over-conservative policies
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99997     # Slow decay to retain exploration
    ...
    --final_lr_fraction=0.97   # Keep more LR for late adjustment
    ...
    --action_reg_weight=3.5e-3 # Keep light action L2 early
    --action_reg_cutoff=3500   # Stop action L2 slightly earlier to let variance rise
...
python run.py "${args[@]}" -name "SwingOption_20_v31_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v31_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v31_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v31_14" -seed 14
