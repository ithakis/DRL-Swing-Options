...
def fit_lsm_estimators(
    ...
    _, S, _, _ = dataset
    prices = np.asarray(S, dtype=np.float64)
    n_paths, n_steps = prices.shape
    ...
    cost_exp = contract.gamma_cost
    exercise_cost_qmax = cost_coeff * (qmax**cost_exp)
    rights = int(round(contract.Q_max / qmax))
    ...
    chebyshev_params = np.zeros((n_steps - 1, 2), dtype=np.float64) if basis == "chebyshev" else None

    for j in range(n_steps - 2, -1, -1):
        price = prices[:, j]
        payoff_gross = qmax * np.maximum(price - strike, 0.0)
        payoff_net = payoff_gross - exercise_cost_qmax
        mask = payoff_gross > 0.0

        X_poly, params = _build_lsm_basis(price, basis, poly_degree)
        if chebyshev_params is not None:
            chebyshev_params[j, 0] = params[0]
            chebyshev_params[j, 1] = params[1]
        ...
        for c in range(cooldown + 1):
            for r in range(1, rights + 1):
                c_keep = max(c - 1, 0)
                y_keep = df * old_vals[c_keep, r]
                beta_keep = _fit_beta(X_poly, y_keep, poly_degree, mask, reg, reg_alpha)
                betas_keep[j, c, r] = beta_keep
                cont_keep = X_poly @ beta_keep

                if c == 0:
                    c_ex = cooldown
                    y_ex = df * old_vals[c_ex, r - 1]
                    beta_ex = _fit_beta(X_poly, y_ex, poly_degree, mask, reg, reg_alpha)
                    betas_ex[j, c, r] = beta_ex
                    cont_ex = X_poly @ beta_ex
                    exc = (payoff_net + cont_ex > cont_keep) & (payoff_gross > 0.0)
                    new_vals[c, r] = np.where(exc, payoff_net + y_ex, y_keep)
                else:
                    new_vals[c, r] = y_keep
        values = new_vals
...
def price_swing_option_lsm_oos(
    ...
    rights = estimators.rights
    cost_coeff = contract.c_cost
    cost_exp = contract.gamma_cost
    exercise_cost_qmax = cost_coeff * (qmax**cost_exp)

    values = np.zeros((cooldown + 1, rights + 1, n_paths), dtype=np.float64)
    exercise = np.zeros((cooldown + 1, rights + 1, n_paths, n_steps), dtype=bool)
    ...
    for j in range(n_steps - 2, -1, -1):
        ...
        if estimators.basis_type == "chebyshev":
            params = tuple(estimators.chebyshev_params[j])
            X_poly, _ = _build_lsm_basis(price, estimators.basis_type, estimators.poly_degree, params)
        else:
            X_poly, _ = _build_lsm_basis(price, estimators.basis_type, estimators.poly_degree)
        ...
        for c in range(cooldown + 1):
            for r in range(1, rights + 1):
                c_keep = max(c - 1, 0)
                y_keep = df * old_vals[c_keep, r]
                beta_keep = estimators.betas_keep[j, c, r]
                cont_keep = X_poly @ beta_keep

                if c == 0:
                    c_ex = cooldown
                    y_ex = df * old_vals[c_ex, r - 1]
                    beta_ex = estimators.betas_ex[j, c, r]
                    cont_ex = X_poly @ beta_ex
                    exc = (payoff_net + cont_ex > cont_keep) & (payoff_gross > 0.0)
                    exercise[c, r, exc, j] = True
                    new_vals[c, r] = np.where(exc, payoff_net + y_ex, y_keep)
    ...
    rights_left = np.full(n_paths, rights, dtype=int)
    ...
    if _print_results:
        print(f"Swing option price: {price_estimate:.4f}\n95% CI: [{ci_low:.4f}, {ci_high:.4f}]")
        print(
            f"LSM settings -> basis: {estimators.basis_type}, degree: {estimators.poly_degree}, "
            f"reg: {estimators.reg_type}, alpha: {estimators.reg_alpha:.2e}"
        )
