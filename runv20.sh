#!/bin/bash
# v20: keep early noise high but taper later for cleaner convergence; slightly slower LR decay for late exploitation.
args=(
    ...
    --noise_sigma=1.25         # Stronger initial noise scale for early exploration
    --noise_anneal_power=0.65  # Tie noise std to epsilon with mild annealing
    --noise_plateau=1500       # Hold initial noise/epsilon for first 1.5k episodes, then decay
    --min_action_noise=0.12    # Higher noise floor to prevent variance collapse
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99994     # Post-plateau decay; leaves modest noise late for exploitation
    ...
    --final_lr_fraction=0.8    # Slower LR decay to allow late-stage adjustment
...
python run.py "${args[@]}" -name "SwingOption_20_v20_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v20_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v20_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v20_14" -seed 14
