#!/bin/bash
# NP1: No PER, stronger early action reg, longer warmup, slightly longer plateau/slower decay to boost late delta, with target smoothing.
args=(
    ...
    -nstep=1
    --per_alpha=0.0            # PER disabled: alpha 0 for uniform sampling
    --per_beta_start=1.0       # PER disabled: beta=1 for uniform importance sampling
    --per_beta_frames=1        # PER disabled: no anneal
    --per_priority_floor=0.0   # PER disabled
    --per_priority_clip_pct=0  # PER disabled
    ...
    -epsilon_decay=0.99997     # Slower decay; retains exploration deeper into training
    -per=0                     # Enable Prioritized Experience Replay
...
python run.py "${args[@]}" -name "SwingOption_20_NP1_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_NP1_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_NP1_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_NP1_14" -seed 14
