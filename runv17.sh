...
args=(
    ...
    -munchausen=0              # Enable Munchausen RL (adds entropy bonus in reward)
    -nstep=1
    ...
    --min_action_noise=0.15    # Minimum action noise to avoid zero-variance policies
    ...
    --tau_final=-1             # Final tau for target schedule (<0 disables)
    --tau_schedule_frac=0.0    # Fraction of training to decay tau toward tau_final
    ...
    --action_reg_weight=1e-3   # L2 regularization on actions to discourage saturation
    --target_policy_noise=0.1  # Target policy smoothing noise std (0 disables)
...
python run.py "${args[@]}" -name "SwingOption_20_v17_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v17_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v17_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v17_14" -seed 14
