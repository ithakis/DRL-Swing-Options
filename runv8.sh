...
args=(
    ...
    --per_priority_floor=2e-5  # Minimum PER priority
    --per_priority_clip_pct=99.7 # Clip PER priorities to percentile (0 disables)
    ...
    --noise_anneal_power=0.6   # Exponent tying noise std to epsilon
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99992     # Epsilon decay factor per episode (sustained exploration)
    ...
    -t=0.0035                  # Target network soft-update rate tau (moderate smoothing)
    ...
    --final_lr_fraction=0.65   # Final learning rate as fraction of initial (decay to 65%)
...
python run.py "${args[@]}" -name "SwingOption_20_v8_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v8_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v8_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v8_14" -seed 14
