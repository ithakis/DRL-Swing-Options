#!/bin/bash
# v21: raise noise floor and extend plateau to prevent late-stage action variance collapse; slow epsilon decay slightly.
args=(
    ...
    --noise_anneal_power=0.6   # Tie noise std to epsilon with mild annealing
    --noise_plateau=2200       # Hold initial noise/epsilon longer to avoid early decay
    --min_action_noise=0.18    # Higher noise floor to prevent variance collapse
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99995     # Post-plateau decay; leaves more noise late for exploitation
...
python run.py "${args[@]}" -name "SwingOption_20_v21_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v21_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v21_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v21_14" -seed 14
