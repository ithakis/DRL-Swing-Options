...
parser.add_argument("--jit", type=int, default=0, choices=[0,1], help="Enable PyTorch JIT compilation for faster inference, default=0 (NO!)")
...
if __name__ == "__main__":
    ...
    # Optimize PyTorch performance settings
    if args.jit:
        # Enable optimizations for JIT compilation
        torch.jit.set_fusion_strategy([('STATIC', 2)])
        if torch.cuda.is_available():
            torch.backends.cudnn.benchmark = True
            torch.backends.cudnn.deterministic = False
        print("JIT optimizations enabled")
    ...
    agent = Agent(state_size=state_size, action_size=action_size, n_step=args.nstep, per=args.per, munchausen=args.munchausen,distributional=args.iqn,
                 ...
                 LR_ACTOR=LR_ACTOR, LR_CRITIC=LR_CRITIC, TAU=TAU, LEARN_EVERY=args.learn_every, LEARN_NUMBER=args.learn_number, device=device, frames=args.frames, worker=args.worker, jit_compile=bool(args.jit)) 
