#!/bin/bash
# v45: v43 baseline, but swap LayerNorm -> RMSNorm (--norm=rmsnorm) to test stability/learning dynamics.
args=(
    ...
    --per_priority_clip_pct=99.7   # Clip extreme priorities to curb spikes while allowing spread
    ...
    --norm=rmsnorm                 # v45 experiment: RMSNorm instead of LayerNorm
...
python run.py "${args[@]}" -name "SwingOption_20_v45_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v45_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v45_13" -seed 13
# python run.py "${args[@]}" -name "SwingOption_20_v45_14" -seed 14
