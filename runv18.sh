#!/bin/bash
# v18: plateaued noise with tuned decay to match the no-plateau end-of-training noise (min_action_noise lowered to 0.05).
args=(
    ...
    --noise_plateau=2000       # Keep epsilon/noise at initial level for the first 2k episodes, then decay
    --min_action_noise=0.05    # Lower noise floor; decay tuned to match no-plateau end-of-training noise
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99992545  # Faster post-plateau decay so final noise matches the no-plateau baseline
...
python run.py "${args[@]}" -name "SwingOption_20_v18_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v18_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v18_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v18_14" -seed 14
