...
echo "Running Longstaff-Schwartz pricing with ${args[1]#*=} Monte Carlo paths..."

# Run the LSM pricing algorithm
python src/longstaff_schwartz_pricer.py "${args[@]}"

echo "=== LSM Pricing Complete ==="
echo "Results saved to lsm_results/ directory"
echo ""
echo "To compare with RL results, check the option values:"
echo "- LSM value is the theoretical benchmark"
echo "- RL agent should converge towards this LSM value"
echo ""
echo "Key files generated:"
echo "- lsm_results/MonthlySwing_LSM_lsm_results.txt (detailed results)"
