#!/bin/bash
# Trial: tuned from v7 baseline to break delta_percent ceiling while keeping stability.
args=(
    ...
    -nstep=1
    --per_alpha=0.52           # PER prioritization exponent (slightly higher focus)
    --per_beta_start=0.78      # PER initial importance-sampling bias correction
    --per_beta_frames=150000   # Anneal beta to 1.0 over 150k transitions
    --per_priority_floor=1.5e-5  # Minimum PER priority (guard against flat priorities)
    --per_priority_clip_pct=99.8 # Clip PER priorities to percentile (0 disables)
    ...
    --noise_anneal_power=0.65  # Exponent tying noise std to epsilon (keeps variance alive)
    ...
    --min_replay_size=16000    # Warm-up buffer size before learning starts (random play)
    ...
    -lr_c=1.65e-4              # Critic learning rate (tempered critic)
    --final_lr_fraction=0.75   # Final learning rate as fraction of initial (decay to 75%)
    ...
    --safe_epsilon_reset=0.2   # Epsilon bump when stuck
...
python run.py "${args[@]}" -name "SwingOption_20_v10_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v10_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v10_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v10_14" -seed 14
