#!/bin/bash
# v36: Keep pre-squash noise; tighten late PER bias and smooth learning rates while adding PER clip to reduce outlier spikes.
args=(
    ...
    --per_priority_clip_pct=99.5   # New: clip extreme priorities to curb late spikes
...
python run.py "${args[@]}" -name "SwingOption_20_v36_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v36_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v36_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v36_14" -seed 14
