...
\end{aligned}
\]
The display above lists the raw economic quantities; Table~\ref{tab:state_components} details the meaning and preprocessing applied to each component before network ingestion (e.g., log-moneyness and robust scaling). The state contains all variables required to make the problem Markov. The inclusion of both used and remaining volume fractions is redundant from a purely mathematical viewpoint, but empirically it stabilises network training. \textcolor{red}{\textbf{Sven:} While noted as practically useful, the inclusion of redundant state variables ($Q_i/Q_{\max}$ vs $(Q_{\max}-Q_i)/Q_{\max}$, and $t_i$ vs $i/n$) is theoretically not super elegant. Worth it to reduce to a minimal sufficient basis to strengthen the theoretical presentation?}
...
The state variable $\rho_i \in [0,1]$ records the normalised number of steps since the last exercise and acts as a soft proximity signal for refraction-style constraints.
...
\end{cases}
\]
The schedule starts at 1.30, decays no lower than 0.26, and only begins to decay after 3{,}200 episodes. % P2-5: Clarify that adaptive noise scales the std of the *pre-squash* noise by (1 + 0.6*|u_i|),
...
The actor output uses $\operatorname{sig}(\beta_{\mathrm{act}} u)$ with $\beta_{\mathrm{act}} = 3.0$, where $\operatorname{sig}$ denotes the logistic sigmoid. State preprocessing is domain-specific: the spot component is mapped to log-moneyness $\log(S_{t_i}/K)$, while the latent factors $X_{t_i}$ and $Y_{t_i}$ are robust-scaled using the sample median and interquartile range. Hidden-layer weights are initialised orthogonally with gain $\sqrt{2}$, and the output layers use a small uniform initialisation in $[-0.003,0.003]$.
...
\caption{Expected exercise quantity $\mathbb{E}[q_t]$ under the continuous DRL policy and the bang-bang LSM baseline for a cost coefficient $c=0.04$ and varying convexity parameters $\gamma_c \in \{1.0, 1.5, 2.0, 3.0\}$. The heatmap displays the average fraction of capacity exercised at each available time step (measured in days to contract maturity) as a function of moneyness ($S/K$). Under linear costs ($\gamma_c = 1$), both policies exhibit bang-bang behavior at high moneyness. As convexity increases, the actor--critic network learns to exercise smaller, continuous quantities across a broader range of states to circumvent large non-linear penalties, whereas the LSM baseline is unable to represent interior optimal controls.}
...
\caption{Net exercise profit versus spot price (left column) and path-level PV exercise cost distribution (right column) for linear ($\gamma_c=1.0$) and convex ($\gamma_c \in \{1.5, 2.0\}$) costs at $c=0.04$. The left column scatters the non-discounted one-step net exercise profit, $q_t(S_t-K)^+ - c q_t^{\gamma_c}$, over the spot price for decisions where exercising full capacity is feasible. Under linear costs, the LSM and RL policies tightly track the same bang-bang boundary. However, under convexity, an area of profitable partial exercise emerges below the upper bang-bang frontier, which the continuous RL agent exploits but the discrete LSM baseline cannot. The right column displays the distribution of total discounted exercise costs per path and tracks the Bang-Bangness metric $B$. As convexity grows, the RL agent mitigates severe cost penalties by reducing its Bang-Bangness and migrating to more distributed exercise strategies, thus maintaining profitability.}
...
To quantify the degree to which a learned policy adheres to the all-or-nothing exercise decisions typical of linear cost structures, we introduce the \emph{Bang-Bangness} metric $B$. We define $B$ as the proportion of feasible full-capacity exercises that actually hit the maximum allowable quantity. Specifically, over the subset of steps where the remaining capacity is at least the maximum single-step limit ($Q_{\max} - Q_i \ge q_{\max}$) and the agent chooses to exercise a strictly positive amount ($q_t > 0$), the metric is defined via the Hit-Ratio:
\begin{equation}
B = \mathbb{P}\left( \frac{q_t}{q_{\max}} \ge 0.95 \;\middle|\; Q_{\max} - Q_i \ge q_{\max},\, q_t > 0 \right).
\end{equation}
A strictly bang-bang policy (such as our LSM baseline) yields $B=100\%$, while a completely uniform random exercise decision over $(0, q_{\max}]$ would yield an expected baseline of $B \approx 5\%$.

\begin{figure}
\centering
\includegraphics[width=\columnwidth]{../figs/convex_costs_0p04/bang_bangness_rl.pdf}
\caption{The Bang-Bangness metric $B$ across different cost regimes and convexities for the RL agent. This validates the distinct downward trend observed in the right column of Figure~\ref{fig:spot_income_pv_hist}; agents systematically decrease the proportion of maximum-capacity exercises to avoid escalating marginal penalties as the cost exponent ($\gamma$) grows.}
\label{fig:bang_bangness_trend}
\end{figure}

Figure~\ref{fig:spot_income_pv_hist} provides a microscopic view of the exercise behavior that drives the performance gap observed in Table~\ref{tab:results}. The left column scatters the non-discounted one-step net exercise profit against the underlying spot price. When $c > 0$ and costs are linear ($\gamma_c=1.0$), the LSM baseline is highly effective; both RL and LSM agents form a tight, linear exercise boundary, proving that bang-bang exercise is optimal. 

However, under convex costs ($\gamma_c \in \{1.5, 2.0\}$), a dense region of profitable partial exercise emerges beneath the strict bang-bang frontier. Because the baseline LSM is structurally restricted to $0$ or $q_{\max}$ decisions, it cannot access this non-linear interior region. Consequently, it often receives highly penalised terminal payouts. The actor--critic RL agent, conversely, continuously modulates its exercise quantity, successfully exploiting these intermediate profitable states.

This behavioral shift is corroborated by the right column of Figure~\ref{fig:spot_income_pv_hist}, which depicts the distribution of the path-level present value of exercise costs. At $\gamma_c=1.0$, the RL agent exhibits high Bang-Bangness ($B \approx 61\%$) and its cost distribution closely tracks that of the LSM baseline. As the cost exponent increases to $\gamma_c = 2.0$, the RL agent's Bang-Bangness decays markedly to $B \approx 14\%$. This reduction in $B$ coincides visually and numerically with the agent incurring substantially lower maximum exercise costs than its LSM counterpart. By picking precisely when and how much to exercise, the agent maintains overall profitability despite the stringent convex penalties.

\subsection{Discussion}

Three qualitative patterns stand out in Table~\ref{tab:results} and Figures~\ref{fig:exercise_heatmap} and~\ref{fig:spot_income_pv_hist}.
\begin{enumerate}
...
The authors gratefully acknowledge financial support from the Amsterdam University Fund through the AUF Impact Call (Fall 2024) and additional support from the AI4FinTech initiative at the University of Amsterdam.
