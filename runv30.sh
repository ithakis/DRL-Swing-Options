#!/bin/bash
# v30: Sigmoid PER ramp, slightly lower late alpha, quicker ramp end, and a bit more late LR to lift final delta while keeping early robustness.
args=(
    ...
    --per_alpha_final=0.30     # Slightly lower final alpha to reduce late over-focus
    --per_alpha_ramp_start=3000  # Start PER alpha ramp after ~3.0k episodes
    --per_alpha_ramp_end=13000   # End PER alpha ramp by ~13k episodes
    ...
    --noise_sigma=1.3          # Back to v26 noise
    --noise_anneal_power=0.55  # Gentle annealing tied to epsilon
    --noise_plateau=3200       # Back to v26 plateau
    --min_action_noise=0.18    # Floor to avoid collapse
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99997     # Back to v26 decay
    ...
    --action_reg_weight=3.5e-3 # Slightly lighter action L2 early
...
python run.py "${args[@]}" -name "SwingOption_20_v30_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v30_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v30_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v30_14" -seed 14
