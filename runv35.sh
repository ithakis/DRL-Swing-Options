#!/bin/bash
# v35: Pre-squash noise kept, but softened late PER and lower LR/noise floor to reduce post-20k seed divergence.
args=(
    ...
    --per_alpha_final=0.32     # Softer late PER (from 0.4) to curb outlier dominance after 20k
    --per_alpha_ramp_start=5000  # Start PER alpha ramp after ~5k episodes
    --per_alpha_ramp_end=18000   # Extend ramp (from 15k) to keep replay closer to uniform through mid-run
    --per_beta_final=0.95      # Stronger late IS correction (from 0.8) to reduce PER bias
    ...
    -noise_floor=0.24     # Lower floor (from 0.32) to temper late action variance
    ...
    -lr_a=2.0e-4               # Lower actor LR (from 2.2e-4) to stabilize late-stage updates
    -lr_c=1.1e-4               # Lower critic LR (from 1.2e-4) to reduce late TD noise
    --final_lr_fraction=0.90   # More decay by the end (from 0.95) to shrink late gradients
...
python run.py "${args[@]}" -name "SwingOption_20_v35_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v35_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v35_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v35_14" -seed 14
