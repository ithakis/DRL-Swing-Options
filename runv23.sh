#!/bin/bash
# v23: keep softer PER, extend plateau and exploration, and gate strong action reg to the first 4k episodes.
args=(
    ...
    -nstep=1
    --per_alpha=0.45           # Softer PER to reduce over-focus
    ...
    --noise_plateau=3000       # Hold initial noise/epsilon longer to avoid early decay
    --min_action_noise=0.20    # Noise floor to prevent variance collapse but allow more exploitation late
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.999965    # Slower decay; retains exploration deeper into training
    ...
    --final_lr_fraction=0.9    # Keep more learning rate for late-stage adjustment
    ...
    --action_reg_weight=5e-3  # Stronger action L2 early
...
python run.py "${args[@]}" -name "SwingOption_20_v23_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v23_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v23_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v23_14" -seed 14
