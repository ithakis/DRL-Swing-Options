...
args=(
    ...
    --noise_anneal_power=0.8   # Exponent tying noise std to epsilon
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99995     # Epsilon decay factor per episode (maintain more exploration)
    ...
    -t=0.0030                  # Target network soft-update rate tau (moderate smoothing)
...
python run.py "${args[@]}" -name "SwingOption_20_v6_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v6_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v6_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v6_14" -seed 14
