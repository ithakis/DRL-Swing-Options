#!/bin/bash
# Trial: v7-style stable baseline with tweaks to avoid saturation and improve delta_percent.
args=(
    ...
    -nstep=1
    --per_alpha=0.55           # PER prioritization exponent
    ...
    --per_priority_clip_pct=0.0 # Clip PER priorities to percentile (0 disables)
    ...
    --noise_sigma=1.2          # Scale exploration noise (decays with epsilon)
...
python run.py "${args[@]}" -name "SwingOption_20_v11_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v11_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v11_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v11_14" -seed 14
