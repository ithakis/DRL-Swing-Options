#!/bin/bash
# Trial: v7 baseline with larger batch (128) and scaled hyperparameters to preserve learning dynamics.
args=(
    ...
    -nstep=1
    --per_alpha=0.5            # PER prioritization exponent (slightly softer than v11 to reduce oversampling spikes)
    ...
    --per_priority_floor=5e-6  # Minimum PER priority
    ...
    --noise_sigma=1.1          # Scale exploration noise (decays with epsilon)
    --noise_anneal_power=0.55  # Exponent tying noise std to epsilon
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99993     # Epsilon decay factor per episode (sustained exploration)
    -per=1                     # Enable Prioritized Experience Replay
    --min_replay_size=24000    # Warm-up buffer size before learning starts (larger for bigger batch)
    ...
    --safe_training=1          # Enable stagnation watchdog
    --safe_window=800          # Episodes to monitor variance over
    --safe_var_threshold=1e-3  # Variance threshold for stuck detection
    --safe_delta_threshold=0.05 # Min (max-min) over window; below => stuck
    --safe_action_var_threshold=5e-3 # Action variance threshold for stuck-policy detection
    --safe_action_upper_threshold=0.9 # Upper-bound action fraction indicating saturation
    --safe_action_window=200   # Episodes to monitor action stats
    --safe_epsilon_reset=0.25  # Epsilon bump when stuck
    --safe_lr_boost=1.15       # Multiply current lr by this on intervention (capped at init)
...
python run.py "${args[@]}" -name "SwingOption_20_v12_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v12_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v12_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v12_14" -seed 14
