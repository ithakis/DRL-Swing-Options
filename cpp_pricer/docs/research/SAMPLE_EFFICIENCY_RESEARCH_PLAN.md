...
*Status: P0+P1 COMPLETE — cheap arms (A1/A2/A4) screened across all 3 regimes, full negative (see §7b).
A3 (quantization tree) gated and not built. v0.2 — June 2026. Harness/tooling retained for revisit.*
...
## 7b. P1 results (cheap screen A1/A2/A4 — COMPLETE, all 3 regimes, 2026-06-15)

**Harness:** `--sampler {mc,arqmc}` + `--replay {uniform,time,coverage}` + `--step_density` in the
v65 C++ pricer (`mc/uniform` bit-identical to v65, verified seed 11 → 1.984538). Screen:
**nocost / g1 / g2** × N∈{1024,2048,4096} × 12 seeds (11–22), common 65 536-path MC test set.
Collector `tools/collect_sampler_results.py`; analysis `tools/analyze_sampler_screen.py` (paired-seed
t, F variance-ratio, paired TOST ±0.5%). Results: `p1_screen.csv` (g2), `p1_confirm.csv` (nocost/g1).

g2 focal (paired Δprice vs A0; seed-std ratio arm/A0):

| arm | N=1024 | N=2048 | N=4096 | std-ratio | verdict |
|-----|-------:|-------:|-------:|-----------|---------|
| A1 array-RQMC      | **−0.0726** | **−0.0334** | **−0.0203** | 3.88 / 1.30 / 1.61 | **REJECT** (worse, higher var) |
...
Cross-regime confirm (g1, nocost) — **same picture, A1 worse everywhere and destabilizes hardest in
nocost** (no convex-cost gate to stabilize the policy): A1 std-ratio reaches **20–24×** in nocost
N∈{1024,4096}; A2 inherits one nocost blow-up (std 24× at N=1024); A4 stays tie-only. **Promotions
across all 27 cells: A1 0/9, A2 0/9, A4 0/9.**

**Conclusion:** *No arm promoted in any regime.* MC + uniform replay is already on the sample-efficiency
frontier for this pricer at N≥1024. Re-weighting the replay distribution (A2/A4) is statistically
*equivalent* to uniform — no higher price, no tighter seed-std. Array-RQMC (A1) **actively hurts**:
sorting the chains by state each step and driving them with a shared rank-stratified point set
correlates the per-step transitions, shrinking the effective replay diversity the off-policy critic
needs — worst at small N (where coverage should help most) and catastrophic in nocost. The user's
state-geometry intuition (sparse-early/dense-late) is correct but does not translate into a better
trainer at this (already-minimal) net size — consistent with the v65 "net is minimal / training
converged" finding.

Decision: A1/A2/A4 closed (negative). **A3 (optimal-quantization tree) gated and NOT built** — it is
the high-cost arm and three independent coverage mechanisms failing (the closest, A1, failing badly)
lowers its prior. The harness + tooling remain to revisit A3 cheaply if the network is ever scaled up
(where coverage may begin to bind). Written up in `HPT.md` "v66 sampling". **No change to v65 canonical.**
