#!/bin/bash
# v25: PER soft/near-uniform early, keep fast early convergence, add more late exploitation (lower noise floor/decay), and slightly lighter action reg.
args=(
    ...
    -nstep=1
    --per_alpha=0.2            # PER very soft (near-uniform early)
    --per_beta_start=1.0       # Full IS correction to keep PER effect minimal early
    --per_beta_frames=60000    # Slow anneal (stays near 1 for most of training)
    --per_priority_floor=1e-6  # Minimal floor
...
python run.py "${args[@]}" -name "SwingOption_20_v25_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v25_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v25_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v25_14" -seed 14
