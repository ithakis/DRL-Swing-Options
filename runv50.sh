#!/bin/bash
# v50: v49 + LAP retune for robustness (keep v48 schedule; adjust κ/floor/clip).
args=(
    ...
    --per_huber_kappa=0.5          # More robust tail damping across HHK/swing shifts
    --per_priority_floor=0.05      # Keep some focusing; avoid near-uniform replay from floor=1.0
    --per_priority_clip_pct=99.7   # Safety rail for residual spikes (now applied in code)
...
python run.py "${args[@]}" -name "SwingOption_20_v50_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v50_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v50_13" -seed 13
# python run.py "${args[@]}" -name "SwingOption_20_v50_14" -seed 14
