#!/bin/bash
# v19: keep noise plateau but restore higher noise floor and slightly stronger noise to combat early saturation.
args=(
    ...
    -nstep=1
    --per_alpha=0.5            # PER prioritization exponent (slightly softer to reduce oversampling spikes)
    ...
    --noise_sigma=1.2          # Stronger initial noise scale
    --noise_anneal_power=0.6   # Tie noise std to epsilon with mild annealing
    --noise_plateau=2000       # Hold initial noise/epsilon for first 2k episodes, then decay
    --min_action_noise=0.15    # Higher noise floor to avoid variance collapse
    -epsilon=0.3               # Initial epsilon-greedy exploration probability (30% random actions)
    -epsilon_decay=0.99993     # Post-plateau decay; keeps more noise late than v18
...
python run.py "${args[@]}" -name "SwingOption_20_v19_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v19_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v19_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v19_14" -seed 14
