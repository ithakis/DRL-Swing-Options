#!/bin/bash
# v29: Sigmoid PER ramp with gentler late PER, early tuned for stability like v26, aiming for stronger late delta without collapse.
args=(
    ...
    --per_alpha_final=0.32     # Lower final alpha to reduce late over-focus
    ...
    --final_lr_fraction=0.96   # Slightly higher LR retained for late adjustment
...
python run.py "${args[@]}" -name "SwingOption_20_v29_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v29_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v29_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v29_14" -seed 14
