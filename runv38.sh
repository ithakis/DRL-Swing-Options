#!/bin/bash
# v38: Cosine LR horizon 65k with 1k-episode warmup, higher final LR, and slightly stronger late PER focus.
args=(
    ...
    --per_priority_clip_pct=99.7   # Slightly looser clip to allow modest priority spread
    --per_alpha_final=0.33         # Slightly stronger late PER to tighten focus
    ...
    --per_beta_final=0.98          # Mild IS correction to keep some prioritization effect late
    ...
    -lr_a=2.0e-4                   # Restore slightly higher actor LR for stronger early updates
    -lr_c=1.1e-4                   # Restore slightly higher critic LR for stronger early updates
    --final_lr_fraction=0.08       # Cosine decay to 8% of initial LR by the 65k-episode horizon (more late LR)
...
python run.py "${args[@]}" -name "SwingOption_20_v38_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v38_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v38_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v38_14" -seed 14
