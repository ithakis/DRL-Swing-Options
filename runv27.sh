#!/bin/bash
# v27: Even more uniform early (target strong start by ~2–3k), then ramp PER by ~14–15k; slightly stronger action reg guard and higher initial noise to accelerate early gains.
args=(
    ...
    --per_alpha_ramp_start=2500  # Start PER alpha ramp after ~2.5k episodes
    --per_alpha_ramp_end=14000   # End PER alpha ramp by ~14k episodes
    ...
    --noise_sigma=1.35         # Slightly higher initial noise to boost early exploration
    --noise_anneal_power=0.55  # Gentle annealing tied to epsilon
    --noise_plateau=3500       # Hold initial noise/epsilon longer to avoid early decay
    ...
    -epsilon_decay=0.999975    # Slower decay early to keep exploration high through 3–5k
    ...
    --final_lr_fraction=0.96   # Keep more learning rate for late-stage adjustment
    ...
    --action_reg_weight=4.5e-3  # Slightly stronger early guard
    --action_reg_cutoff=4500    # Keep guard slightly longer
...
python run.py "${args[@]}" -name "SwingOption_20_v27_11" -seed 11 &
python run.py "${args[@]}" -name "SwingOption_20_v27_12" -seed 12 &
python run.py "${args[@]}" -name "SwingOption_20_v27_13" -seed 13 &
python run.py "${args[@]}" -name "SwingOption_20_v27_14" -seed 14
